...
void ensureDeviceBuffer(T*& ptr, size_t& capacity_bytes, size_t need_bytes) {
  ...
  if (ptr != nullptr) {
    RUNTIME_CHECK(musaFree(ptr));
    ptr = nullptr;
  }
  RUNTIME_CHECK(musaMalloc(&ptr, need_bytes));
...
void rmsNorm(const std::vector<T>& h_input, const std::vector<T>& h_weight,
  ...
  RUNTIME_CHECK(musaMemcpy(d_input, h_input.data(), n * sizeof(T),
  ...
  RUNTIME_CHECK(musaMemcpy(h_output.data(), d_output, n * sizeof(T),
  ...
  for (int idx = tid; idx < q_rows * head_dim; idx += blockDim.x) {
    ...
    if (!isfinite(sM[r]) || !(sL[r] > 0.0f)) {
      ...
      O[q_offset * head_dim + d] =
          storeFromFloat<T>(static_cast<float>(static_cast<double>(sO[r * head_dim + d]) /
                                               static_cast<double>(sL[r])));
    }
  }
}
...
__global__ void flashAttentionKernel<float>(const float* Q, const float* K, const float* V, float* O,
  ...
  float* sK = sQ + kFlashBr * head_dim;
  float* sV = sK + kFlashBc * head_dim;
  double* sO = reinterpret_cast<double*>(sV + kFlashBc * head_dim);
  float* sS = reinterpret_cast<float*>(sO + kFlashBr * head_dim);
  float* sM = sS + kFlashBr * kFlashBc;
  double* sL = reinterpret_cast<double*>(sM + kFlashBr);
  ...
  for (int idx = tid; idx < q_rows * head_dim; idx += blockDim.x) {
    ...
    const size_t q_offset =
        (static_cast<size_t>(b) * target_seq_len + t) * query_heads + hq;
    sQ[r * head_dim + d] = loadAsFloat<float>(Q[q_offset * head_dim + d]);
    sO[r * head_dim + d] = 0.0;
  }
  if (tid < q_rows) {
    sM[tid] = -INFINITY;
  ...
  for (int k0 = 0; k0 < src_seq_len; k0 += kFlashBc) {
    ...
    for (int idx = tid; idx < tile_n * head_dim; idx += blockDim.x) {
      ...
      const size_t kv_offset =
          (static_cast<size_t>(b) * src_seq_len + s) * kv_heads + hkv;
      sK[s_local * head_dim + d] = loadAsFloat<float>(K[kv_offset * head_dim + d]);
    }
    __syncthreads();
    ...
    for (int idx = tid; idx < q_rows * tile_n; idx += blockDim.x) {
      ...
      if (is_causal && s > t) {
        ...
        score = dotQK<float>(sQ + r * head_dim, sK + s_local * head_dim, head_dim) *
                scale;
      }
      sS[r * kFlashBc + s_local] = score;
    }
  ...
  for (int k0 = 0; k0 < src_seq_len; k0 += kFlashBc) {
    ...
    for (int idx = tid; idx < tile_n * head_dim; idx += blockDim.x) {
      ...
      const size_t kv_offset =
          (static_cast<size_t>(b) * src_seq_len + s) * kv_heads + hkv;
      sK[s_local * head_dim + d] = loadAsFloat<float>(K[kv_offset * head_dim + d]);
      sV[s_local * head_dim + d] = loadAsFloat<float>(V[kv_offset * head_dim + d]);
    }
    __syncthreads();
    ...
    for (int idx = tid; idx < q_rows * tile_n; idx += blockDim.x) {
      ...
      if (!isfinite(sM[r])) {
        ...
        const float score =
            dotQK<float>(sQ + r * head_dim, sK + s_local * head_dim, head_dim) *
            scale;
        sS[r * kFlashBc + s_local] = expf(score - sM[r]);
      }
    }
    ...
    if (tid < q_rows && isfinite(sM[tid])) {
      double l = sL[tid];
      for (int i = 0; i < tile_n; ++i) {
        l += static_cast<double>(sS[tid * kFlashBc + i]);
      }
      sL[tid] = l;
    }
    ...
    for (int idx = tid; idx < q_rows * head_dim; idx += blockDim.x) {
      ...
      double out = sO[r * head_dim + d];
      for (int i = 0; i < tile_n; ++i) {
        out += static_cast<double>(sS[r * kFlashBc + i]) * sV[i * head_dim + d];
      }
      sO[r * head_dim + d] = out;
    }
    __syncthreads();
  ...
  for (int idx = tid; idx < q_rows * head_dim; idx += blockDim.x) {
    ...
    const size_t q_offset =
        (static_cast<size_t>(b) * target_seq_len + t) * query_heads + hq;
    if (!isfinite(sM[r]) || !(sL[r] > 0.0)) {
      O[q_offset * head_dim + d] = storeFromFloat<float>(0.0f);
    } else {
      O[q_offset * head_dim + d] =
          storeFromFloat<float>(static_cast<float>(sO[r * head_dim + d] /
                                                   sL[r]));
    }
  }
}
...
void flashAttention(const std::vector<T>& h_q, const std::vector<T>& h_k,
  ...
  RUNTIME_CHECK(musaMemcpy(d_q, h_q.data(), q_elems * sizeof(T),
                           musaMemcpyHostToDevice));
  RUNTIME_CHECK(musaMemcpy(d_k, h_k.data(), kv_elems * sizeof(T),
                           musaMemcpyHostToDevice));
  ...
  const size_t sL_bytes = (std::is_same<T, float>::value)
                              ? kFlashBr * sizeof(double)
                              : kFlashBr * sizeof(float);
  const size_t smem_bytes =
      static_cast<size_t>(kFlashBr * head_dim + kFlashBc * head_dim +
                          kFlashBc * head_dim) *
          sizeof(float) +
      sO_bytes +
      static_cast<size_t>(kFlashBr * kFlashBc + kFlashBr) * sizeof(float) +
      sL_bytes;
  ...
  RUNTIME_CHECK(musaMemcpy(h_o.data(), d_o, q_elems * sizeof(T),
                           musaMemcpyDeviceToHost));
}
